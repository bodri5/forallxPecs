...
Semmilyen információval nem kell rendelkeznünk A Három Besúgóról, hogy megállapíthassuk, melyik állítás igaz és melyik hamis a modellben. $Hf$ igaz, mivel $f$ (Larry) referencia $H$ kiterjesztésben is megtalálható. A $\exists x Hx$ és $\exists x \enot Hx$ kifejezések is igazak, hiszen van legalább egy olyan eleme az Uniónak, amely megtalálható $H$ kiterjesztésében, és van legalább egy olyan, amely nincs benne $H$ kiterjesztésében. Így a modell minden formális szignifikanciáját tartalmazza az interpretációnak.
...
Az Unió készlete: $\{1,2,3,4,5,6,7,8,9\}$.
...
Az olyan kiterjesztések, melyek egy-változós predikátumokhoz tartoznak, mint például az $E$ vagy az $N$, csak részhalmazai az Uniónak. Az ilyen kiterjesztésekbe azok az elemek tartoznak, melyekre adott predikátum igaz. Durván mondva az $E$ predikátum kiterjesztése az $E$-k halmaza az Unióban. Az $E$ kiterjesztése $\{2,4,6,8\}$. Rengeteg páros van ezen a négy számon kívül, azonban csak ezek az adott Unió páros elemei. Nincsenek negatív számok ebben az Unióban, tehát $N$ üres kiterjesztésű; $\extension{N}=\emptyset$.
...
Egy két-változós predikátum, mint például $L$, némileg bosszantó. Azt gondolhatnánk, hogy $L$ kiterjesztésének tartalmaznia kéne az 1-et, hiszen 1 az összes többi számnál kisebb; ugyanígy tartalmaznia kéne 2-t, mivel 2 kisebb minden másik számnál 1-et kivéve; és így tovább. Az Unió összes eleme kisebb egy másik elemnél, 9-et kivéve. Mi lenne, ha csak ennyit írnánk: $\extension{L}=\{1,2,3,4,5,6,7,8\}$?
...
A probléma, hogy a halmazok elemeinek sorrendje felcserélhető, így az előbbi kiterjesztés megegyezne a következővel: $\extension{L}=\{8,7,6,5,4,3,2,1\}$. Ebből sajnos nem tudjuk megállapítani, hogy a halmaz melyik eleme kisebb melyik másik elemnél.
...
Valahogy jelölnünk kell, hogy 1 kisebb, mint 8, de 8 nem kisebb, mint 1. A megoldás, hogy $L$ kiterjesztése számpárokból áll. Egy \define{EGYMÁSHOZ RENDELT PÁR}  olyan, mint egy kételemű halmaz, kivéve, hogy a számok sorrendje itt igenis számít. Egymáshoz rendelt párokat `kacsacsőrök' (`$\openntuple$' és `$\closentuple$') közé írjuk.  Az egymáshoz rendelt pár \mbox{\ntuple{foo, bar}} nem egyenlő a \mbox{\ntuple{bar, foo}} párossal. Az $L$ kiterjesztése több ilyen egymáshoz rendelt számpárból áll, mely párok elemei az Unióban megtalálhatóak, és az első elem kisebb, mint a második. Írjuk fel a teljes kiterjesztést:
