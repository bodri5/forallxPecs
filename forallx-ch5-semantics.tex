...
A részleges modell azonban azt mondja nekünk, hogy a $A$ predátum, ami igaz Párizsra és Párizsra. Határtalanul sok olyan predikátum van angolul, amelyek rendelkeznek ezzel a kiterjesztéssel. A $Axy$ ehelyett fordíthatja, hogy a `$x$ azonos méretű, mint a $y$', vagy `$x$ és $y$ egyaránt város.' Hasonlóképpen, a $Bx$ olyan predikátum, amely nem vonatkozik Párizsra; ehelyett fordíthatja: `$x$ egy szigeten van” vagy `$x$ egy subcompact auto.' Amikor megadjuk a $A$ és $B$ kiterjesztéseit, akkor nem határozzuk meg, hogy az angol melyiket predikálja $A$-t és $B$-t kell fordítani. Aggódunk amiatt, hogy a $\forall xAxx \eif Bd$ valóban vagy hamisnak tűnik-e, és a QL-ben az igazság és a hamisság szempontjából fontos a modellben szereplő információ: az UD, a predikátumok kiterjesztései és a hivatkozások állandók száma.
...
Ugyanolyan könnyen meg tudjuk mutatni, hogy a $\forall xAxx \eif Bd$ nem ellentmondás. Csak azt a modellt kell megadnunk, amelyben a $\forall xAxx \eif Bd$ igaz; vagyis egy olyan modell, amelyben a $\forall x Axx$ hamis, vagy $Bd$  igaz. Itt van egy ilyen részleges modell:
\begin{partialmodel}
	UD			& \{Paris\}\\
	\extension{A} 	& \{\ntuple{Paris,Paris}\}\\
	\extension{B}	& \{Paris\}\\
...
Megmutattuk, hogy a $\forall xAxx \eif Bd$ sem tautológia, sem ellentmondás. A 'kontingens QL-ben' meghatározása azt jelenti $\forall xAxx \eif Bd$ függő. Általában annak megállapításához, hogy egy mondat függő, két modellre van szükség: az egyikben a mondat igaz, a másikban pedig a hamis mondat.
...
Megalapozhatjuk, hogy a $\exists x Sx$ -nak igaznak, ha valamit belefoglalunk a $S$ kiterjesztésébe, és ha $\forall x Sx$ -ot hamisnak tesszük, ha valamit kihagyunk a $ S $ kiterjesztéséből. Nem számít, melyiket belefoglaljuk, és melyiket hagyjuk ki. Ha Duke az egyetlen $S$, akkor egy részleges modellt kapunk, amely így néz ki:
\begin{partialmodel}
	UD			& \{Duke, Miles\}\\
	\extension{S}	& \{Duke\}
\end{partialmodel}
...
Ez a részleges modell azt mutatja, hogy a két mondat logikailag \emph{not}  logikailag ekvivalens.

Back on p.~\pageref{surgeon3correct}, we said that this argument would be invalid in QL:

Visszatérve a p.~\pageref{surgeon3correct}, azt mondtuk, hogy ez az érv érvénytelen lenne a QL-ben:
\begin{earg}
