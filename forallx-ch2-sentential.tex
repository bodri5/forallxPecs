...
szükségszerint indexelve & $A_1, B_1,Z_1,A_2,A_{25},J_{375},\ldots$\\
\hline
...
A \define{KL-beli kifejezés}t egy bármely, csak KL-beli szimbólumokat tartalmazó láncként definiáljuk. Ha kiválasztunk egy-, vagy több szimbólumot a KL szimbólumkészletéből, és bármely sorrendben leírjuk őket egymás után, akkor egy kifejezést kapunk.

\subsection{Well-formed formulae}

\subsection{Jól definiált kifejezések}
...
Mivel minden szimbólumsorozatot kifejezésnek tekintetünk, rengeteg kifejezés értelmetlen halandzsa lesz. Az ezen belüli értelmes kifejezések alhalmazára a \emph{jól definiált kifejezés} elnevezést használjuk. Gyakori röviditése a \emph{jdk} (többesszámban: \emph{jdk}-k).
...
Nyilvánvaló módon az egyetlen kijelentésváltozóból álló kifejezések, mint pl $A$ és $G_{13}$ jdk-k lesznek. Ezekből további jdk-kat tudunk formálni a különböző műveletek használatával; negáció használatával például $\enot A$ és $\enot G_{13}$, konjukció használatával pedíg $A \eand G_{13}$, $G_{13} \eand A$, $A \eand A$, és $G_{13} \eand G_{13}$ jdk-kat kapunk. Negáció többszöri használatával $\enot \enot A$, negáció és konjukció használatával pedíg $\enot(A \eand G_{13})$ és $\enot(G_{13} \eand \enot G_{13})$ jdk-kat kaphatunk. A lehetséges kombinációk száma végtelen, még csak ezt a két kijelentésváltozó használatánál is, és ezen felül még végtelen sok kijelentésváltozó is létezik, ezért semmi értelme nincs megpróbálni felsorolni az összes jdk-t.
...
Ehelyett leírjunk a folyamatot amely által jdk-kat tudunk konstruálni. A negáció definiálásának egy módja például: Bármely KL-beli jdk \script{A} esetén $\enot\script{A}$ is egy KL-beli jdk. Fontos megjegyeznünk hogy itt \script{A} nem egy kijelentés-, hanem egy bármely jdk értéket képviselő változó. Vegyük észre, hogy mivel \script{A} nem egy KL-beli szimbólunm, ezért $\enot\script{A}$ sem egy kijelentés a KL-ben; ehelyett egy metanyelvbeli kifejezés, amely megengedi számunkra hogy végtelenül sok KL-beli kifejezésről tudjunk beszélni, esetünkben minden negációval kezdődő kifejezésről. Mivel \script{A} a metanyelv tagja, ezért a hasonló változók elnevezése \emph{metaváltozó}.
