...
%Az Atomi mondatok összekapcsolódnak, hogy összetett mondatokat alkossanak hasonlóképpen, mint a fizikai atomok kapcsolódnak össze, hogy molekulákat alkossanak. A fizikai atomokat eredetileg, azért nevezték "atomoknak", mert a kémikusok azt hitték, hogy azok már nem bonthatóak további részekre. A kémikusok tévedtek, és a fizikai atomokat tovább lehet bontani.
...
%Fontos megjegyeznünk, hogy a szimbólum kulcs csak abból az okból ad értelmet az atomi mondatoknak, hogy egy kifejezett érvelést lefordítson.
...
\factoidbox{
Egy mondatot jelölhetünk $\script{A}\eand\script{B}$-nek, ha elmondható róluk magyarul, hogy „Mind \script{A} és \script{B}”. A konjunktoknak külön-külön is mondatoknak kell lenniük.
}
...
\factoidbox{
Azok a mondatok, amelyek kifejezhetők úgy, mint „\script{A}, de \script{B}” vagy „Bár \script{A}, \script{B}” a legegyszerűbben a következő konjunkcióval jelölhetők: \script{A}\eand\script{B}
}
...
Bármely \script{A} és \script{B}, \script{A}\eand\script{B} mondat akkor és csak akkor igaz, ha mind \script{A}\ és mind \script{B} is igaz. Ezt összefoglalhatjuk a konjunkcióra vonatkozó igazságtáblázatban:
...
\script{A} és \script{B} micsodák, \script{A}\eand\script{B} logikailag egyenlő \script{B}\eand\script{A}-val.
...
A \ref{or1}. mondat lényege „$D$ vagy $M$.” Hogy szimbolizálni tudjuk ezt, megismerkedünk egy új jelöléssel. A mondat $D \eor M$-mé válik. A „\eor” kapcsolatot \define{diszjunkciónak} nevezzük, továbbá $D$ és $M$ \define{diszjunktak} egymásra.
...
\factoidbox{
Egy mondathoz használhatjuk a $\script{A}\eor\script{B}$ jelölést, ha más szóval „\script{A} vagy \script{B}.” Minden diszjunkciónak mondatnak kell lennie.
}
...
Néha a „vagy” kizárja, hogy mindkét diszjunkció igaz legyen. Ezt nevezzük \define{kizáró vagy}-nak. Egy \emph{kizáró vagy} tisztán felismerhető, mikor egy éttermi étlapon azt olvassuk, hogy „A főételekhez leves vagy saláta jár.” Választhatsz levest; választhatsz salátát; de, ha \emph{mindkettőt,} levest \emph{és} salátát is szeretnél, többet kell fizetned. 
...
Máskor a „vagy” megengedi, hogy mindkét diszjunkció igaz legyen. Nézzük ezt az esetet a fenti \ref{or2} mondatokkal. Játszani fogok Dénessel, Erikával, vagy Dénessel és Erikával. A \ref{or2} mondat csupán azt mondja, hogy játszani fogok \emph{legalább} egyikükkel. Ezt \define{megengedő vagy}-nak nevezzük.
...
Ahogy a konjukció, úgy a diszjunkció is szimmetrikus. Az \script{A}\eor\script{B} és a \script{B}\eor\script{A} logikailag ekvivalensek.
...
A következő állítások valamivel komplikáltabbak:
...
\Aref{or3}. állítást úgy lehet értelmezni hogy „Vagy \emph{nem az az eset hogy} levest kapsz, vagy \emph{nem az az eset hogy} salátát kapsz.” Ennek a lefordításához kell diszjunkció és negáció is. Az lesz belőle, hogy: $\enot S_1 \eor \enot S_2$.
...
\Aref{or4}. állítás is igényel negációt. Úgy lehet értelmezni hogy „\emph{nem az az eset hogy} vagy levest kapsz vagy salátát.” Szükségünk van egy módra hogy jelezni tudjuk hogy a negáció nem csak a jobb vagy a bal diszjunkt részt negálja, hanem az egész diszjunkciót. Ilyenkor zárójeleket használhatunk a diszjunkció köré. „Nem az az eset, hogy $(S_1 \eor S_2)$.” Ez egyszerűbben a következőképp néz ki $\enot (S_1 \eor S_2)$.
...
\Aref{or.xor}. mondat egy \emph{kizáró vagy} állítás. Ezt az állítást két részre tudjuk bontani. Az első rész azt mondja, hogy vagy az egyiket kapod, vagy a másikat. Ezt úgy fordíthatjuk le hogy $(S_1 \eor S_2)$. A második rész azt mondja, hogy nem kaphatod meg mindkettőt. Ezt úgy értelmezhetjük hogy „Nem az az eset, hogy kapsz levest és salátát is.” Mind a negációt és konjukciót is használva lefordíthatjuk ezt úgy, hogy $\enot(S_1 \eand S_2)$. Most már csak össze kell raknunk a két oldalt. Ahogy láttuk feljebb, a „de” szót gyakran értelmezhetjük konjukciónak. Ezért \aref{or.xor} állítást lehet úgy fordítani hogy $(S_1 \eor S_2) \eand \enot(S_1 \eand S_2)$.
...
A mondat $R\eif B$ lesz. A  logikai függvényt \define{implikáció}-nak nevezzük. A kijelentést az implikáció bal oldalán (ebben a példában $R$) \define{előfeltétel}-nek
nevezzük.  A kijelentést a jobb oldalon ($B$) \define{következmény}-nek nevezzük.
...
\factoidbox{
Az átalakított „\script{A} csak akkor, ha \script{B}” mondat logikailag megegyezik a „ha \script{A}, akkor \script{B}” kifejezéssel.
}
...
„Ha \script{A}, akkor \script{B}” azt jelenti, hogy ha \script{A} igaz, akkor \script{B} is. Tehát tudjuk, hogy ha
az \script{A} mondatrész igaz, de a következõ \script{B} hamis, akkor az implikáció „ha
 \script{A}, akkor  \script{B}” hamis. Mi az igazságértéke „Ha \script{A} akkor \script{B}”-nek más körülmények között?
Tegyük fel például, hogy az \script{A} mondatrész hamis. A „Ha \script{A}, akkor \script{B}” semmit nem mondana
 nekünk  \script{B} igazságértékről, és az sem tiszta, hogy mi az igazságérteke a „Ha \script{A} akkor \script{B}” mondatnak.
...
Annak érdekében, hogy az állítást KL-re fordítsuk, ne próbáljuk meg megragadni a `Ha$\ldots$ akkor$\ldots$` szerkezet finomságait. Ehelyett a `\eif' szimbólum \emph{anyagi feltétel} lesz. Ez azt jelenti, hogy ha \script{A} hamis, akkor az \script{A}\eif\script{B} állítás automatikusan igaz, függetlenül a \script{B} igazságtartalmától. Ha \script{A} és \script{B} igaz, akkor az \script{A}\eif\script{B} állítás is igaz lesz.
...
Röviden: az \script{A}\eif\script{B} állítás csak akkor lesz hamis, ha \script{A} igaz, de \script{B} hamis. Összefoglalhatjuk ezt egy igazságtáblával.
...
Az állítás \emph{asszimetrikus}. A két állítás nem cserélhető fel a jelentésük megváltoztatása nélkül, mert \script{A}\eif\script{B} és \script{B}\eif\script{A} logikailag nem ekvivalensek.
...
A \ref{iff3} -os mondat azt mondja, ha $T$ igaz \emph{akkor és csak akkor} $S$ is igaz; $S$ magával vonja $T$ -t, és $T$ is $S$ -t. Ezt \define{ekvivalenciának} hívjuk, mert két felételt von maga után $S\eif T$ és $T \eif S$ -t. „\eiff” -t fogjuk használni, hogy kifejezzük az ekvivalenciát; a \ref{iff3} -os mondat lefordítható $S \eiff T$ -ként.
...
\script{A}\eiff\script{B} igaz akkor és csak akkor, ha \script{A} és \script{B} igazságértéke megegyezik. Itt van a jellegzetes igazság tábla az ekvivalenciára.
...
Melyik a helyes fordítása \aref{unless1}. mondatnak? Mindkettő helytálló, mert mindegyik fordítás logikailag ekvivalens a KL-ban.
...
\Aref{unless2}. mondat magyarul logikailag ekvivalens \aref{unless1}. mondattal. Le lehet fordítani $\enot J \eif D$ -nek vagy $\enot D \eif J$ -nek is.
...
\factoidbox{
Ha egy mondat definiálhatunk úgy, hogy `Hacsak \script{A}, \script{B},' Akkor szimbolizálhatjuk a következőképpen $\script{A}\eor\script{B}$.
}

Symbolization of standard sentence types is summarized on p.~\pageref{app.symbolization}.

Az alapvető mondatok szimbolizációja \apageref{app.symbolization}. oldalon van összefoglalva.
...
Ezért fontos különbséget tenni a KL logikai nyelve, amit kialakítunk, és a beszélt nyelvünk között. Amikor egy nyelvről beszélünk, akkor a nyelvet, amiről beszélünk úgy hívjuk, hogy \define{tárgynyelv}. Továbbá a nyelv, amit használunk az elmondására, azt meg \define{metanyelv}nek hívjuk.
\label{def.metanyelv}
...
\nix{A kifejezés fogalmának definiálása önmagában nem tesz sokat, mivel képesek vagyunk létrehozni egy jól definiált kijelentést bármely rájuk való utalás nélkül; viszont néha kényelmes a használatuk.}
...
A \define{KL-beli kifejezés}t egy bármely, csak KL-beli szimbólumokat tartalmazó karakterláncként definiáljuk. Ha kiválasztunk egy, vagy több szimbólumot a KL szimbólumkészletéből, és bármely sorrendben leírjuk őket egymás után, akkor egy kifejezést kapunk.
...
Mivel minden szimbólumsorozatot kifejezésnek tekintetünk, rengeteg kifejezés értelmetlen halandzsa lesz. Az ezen belüli értelmes kifejezések részhalmazára a \emph{jól formált kifejezés} elnevezést használjuk. Gyakori röviditése a \emph{jfk} (többesszámban: \emph{jfk}-k).
...
Ehelyett leírjuk a folyamatot amely által jfk-kat tudunk konstruálni. A negáció definiálásának egy módja például: Bármely KL-beli \script{A} jól formált kifejezés esetén $\enot\script{A}$ is egy KL-beli jfk. Fontos megjegyeznünk hogy itt \script{A} nem egy kijelentésváltozó, hanem egy bármely jfk értéket képviselő változó. Vegyük észre, hogy mivel \script{A} nem egy KL-beli szimbólum, ezért $\enot\script{A}$ sem egy kijelentés a KL-ban; ehelyett egy metanyelvbeli kifejezés, amely megengedi számunkra, hogy végtelenül sok KL-beli kifejezésről tudjunk beszélni, esetünkben minden negációval kezdődő kifejezésről. Mivel \script{A} a metanyelv tagja, ezért a hasonló változók elnevezése \emph{metaváltozó}.
...
Hasonló dolgokat mondhatunk a többi műveletre is. Például, ha \script{A} és \script{B} KL-beli jfk-k, akkor $(\script{A}\eand\script{B})$ is egy KL-beli jfk. Megadva ilyen kikötéseket az összes műveletre, az alábbi formális definíciót kapjuk egy {KL-beli jól formált kifejezéshez}:
...
\item Ha \script{A} egy jfk, akkor $\enot\script{A}$ is egy KL-beli jfk.
\item Ha \script{A} és \script{B} jfk-k, akkor $(\script{A}\eand\script{B})$ is egy jfk.
\item Ha \script{A} és \script{B} jfk-k, akkor $(\script{A}\eor\script{B})$ is egy jfk.
\item Ha \script{A} és \script{B} jfk-k, akkor $(\script{A}\eif\script{B})$ is egy jfk.
\item Ha \script{A} és \script{B} jfk-k, akkor $(\script{A}\eiff\script{B})$ is egy jfk.
...
Azt a műveletet, amelyet az első helyen látsz egy mondat felbontásában, azt a mondat \define{fő logikai műveletének} nevezzük. Például: A $\enot (E \eor (F \eif G))$ fő logikai művelete a negálás, \enot. A $(\enot E \eor (F \eif G))$ fő logikai művelete a diszjunkció, \eor.
...
A mondatok rekurzív struktúrája a KL-ben lesz fontos, amikor figyelembe vesszük annak körülményeit, mikor lesz az adott mondat igaz vagy hamis. A $\enot \enot \enot D$ mondat akkor és csakis akkor igaz, ha a $\enot \enot D$ mondat hamis, és így tovább a mondat struktúráján keresztül amíg nem érünk el az atomi komponensekhez: $\enot \enot \enot D$ akkor és csakis akkor igaz, ha az atomi mondat $D$ hamis. Erre még visszatérünk a következő fejezetben.
...
%\null{Ennek nem kéne meglepetésnek lennie.Az Angol nyelvben vannak jelentőségteljes kifejezések amik nem mondatok. Például, `a legmagasabb hegycsúcs a Himalájában található

\subsection*{Notational conventions}
\subsection{Jelölési szabályok}
\label{SLconventions}
...
A jfk mint $(Q \eand R)$ zárójelekben kell lennie,mert talán használhatjuk a defenicíót megint, hogy része legyen egy még inkább komplikáltabb mondatnak.Ha tagadjuk $(Q \eand R)$, akkor $\enot(Q \eand R)$ kapunk.Ha pedig csak $Q \eand R$ van a zárójelek nélkül és egy tagadá jelet teszünk elé akkor $\enot Q \eand R$ kapnánk.Természetesen, hogy ezt úgy olvassuk mintha ugyanaz lenne mint a $(\enot Q \eand R)$,valami teljesen különböző mint $\enot(Q\eand R)$.A $\enot(Q \eand R)$ azt jelenti, hogy nem igaz $Q$ és $R$ is; $Q$ lehet hamis vagy $R$ lehet hamis, de a mondat nem mondja el, hogy melyik.A $(\enot Q \eand R)$ azt jelenti, hogy pontosan $Q$ hamis és $R$ igaz.A zárójelek végzetesen fontosak a mondat jelentéséhez.
...
Szóval szigorúan értve,$Q \eand R$ a zárójelek nélkül \emph{nem} a mondata a KL-nek. Amikor KL-t hazsnálunk,azonban gyakran megengedjük magunknak, hogy eltekintsünk a pontos jelentéstől, hogy a dolgokat könyebbé tegyük magunk számára. Ezt többféleképpen tehetjük meg.
...
Először is meg kell értenünk, hogy $Q \eand R$ ugyanazt jelenti mint $(Q \eand R)$.Jelölési szempontból lehagyhatjuk a zárójeleket amik az \emph{egész mondat körül vannak}.
...
Másodszor néha zavaró lehet hosszú mondatok a sok beágyazott zárójeleikkel nézni.Alkalmazzuk a szögletes zárójeles jelölést `[' ls `]' a zárójelek helyett.Példaképpen nincsen logikai különbség $(P\eor Q)$ ls  $[P\eor Q]$ között.A kezelhetetlen mondat:
...
\item[A:] Kovács Urat megölték.
\item[B:] The butler did it.
\item[B:] A komornyik tette.
\item[C:] The cook did it.
\item[C:] A szakács tette.
\item[D:] The Duchess is lying.
\item[D:] A hercegnő hazudik.
\item[E:] Mister Edge was murdered.
\item[E:] Szabó Urat megölték.
\item[F:] The murder weapon was a frying pan.
\item[F:] A gyilkos fegyver egy serpenyő volt.
\end{ekey}
\begin{earg}
...
\problempart Adj egy szimbolizációs kulcsot és szimbolizáld az alábbi mondatokat KL-ben.
\begin{earg}
