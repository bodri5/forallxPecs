...
Következőnek tegyük fel, hogy le akarjuk fordítani ezt az érvelést:
\begin{quote}
\label{surgeon3}
...
\begin{quote}
\label{surgeon3}
Carol szakképzett sebés és teniszezik. Ennélfogva Carol egy szakképzett teniszező.
\end{quote}
Ha az előző érvelésben használt szimbolizációs kulccsal kezdünk, hozzáadhatjuk a predikátumot (jelentse $Tx$ hogy $x$ teniszezik') és egy konstansot (legyen $c$ Carol). Ekkor az érvelés a következő:
\begin{earg}
\item[] $(Rc \eand Kc) \ eand Tc$
\item\therefore] $ Tx \eand Kc$
\end{earg}
Ez a fordítás katasztrofális! Veszi a magyar nyelven szörnyű érvelést és lefordítja egy PL-ben érvényes érveléssé. A probléma az, hogy különbség van \emph{szakképzett sebésznek lenni} és \emph{szakképzett teniszezőnek lenni} között. Az érvelés helyes fordításához kétfajta predikátum szükséges, mindkét ismerethez egy. Ha $K_1x$ azt jelenti, hogy '$x$ egy képzett sebész' és $K_2x$ azt, hogy '$x$ képzett teniszező', akkor így szimbolizálhatjuk az érvelést:
\begin{earg}
\label{surgeon3correct}
\item[] $(Rc \eand K_1c) \eand Tc$
\item[\therefore] $Tc \eand K_2c$
...
Pont ahogy a magyar mondat amit lefordít, ez is helytelen %\nix{Vegyük észre, hogy nincs logikai kapcsolat $K_1c$ és $Rx$ között. Mint PL szimbólumok, lehetnek unáris predikátumok. Angol nyelvben van kapcsolat \emph{sebész} és \emph{szakképzett sebész} között: minden szakképzett sebész sebész. Annak érdekében hogy ezt a kapcsolatot megfogalmazzuk, szimbolizáljuk a 'Carol egy szakképzett sebész'-t mint $Rc \eand K_1c$. Ekkor a jelentése: 'Carol egy sebész és szakképzett, mint sebész.'}
...
Ezen példák tanulsága az, hogy nagyon elővigyázatosan fordítsunk kétértelmű predikátumokat. Hasonló problémákba ütközhetünk a \emph{jó}, \emph{rossz}, \emph{nagy}, és \emph{kicsi} predikátumokkal. Ugyanúgy, ahogy egy képzett sebésznek és egy képzett teniszezőnek más jártasságai vannak, egy nagy kutya, nagy egér és egy nagy probléma is másképpen nagy. 
...
Vajon elég egy predikátum, ami azt jelenti, hogy '$x$ egy képzett sebész', ahelyett, hogy két predikátumot használnánk, mint '$x$ képzett' és $x$ sebész'? Néha. Ahogy a \ref{surgeon} mondat mutatja, néha nem kell különbséget tenni képzett sebészek és más sebészek között. 
...
Vajon mindig meg kell különböztessük a képzett, jó, rossz vagy nagy különböző jelentéseit? Nem. Ahogy az érvelés Billyről mutatja, néha elég egy fajta készségről beszélni. Ha egy érvelés csak kutyákról szól, rendben van definiálni egy predikátumot '$x$ nagy' jelentéssel. Azonban ha az UD kutyákat és egereket is tartalmaz, valószínűleg a legjobb egy olyan predikátumot definiálni, ami '$x$ nagy egy kutyához képest'-et jelent.
