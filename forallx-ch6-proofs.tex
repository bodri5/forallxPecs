...
\section{Bizonyítások és modellek}

As you might already suspect, there is a connection between \emph{theorems} and \emph{tautologies}.

Mint már gyaníthatja, kapcsolat áll fenn a \emph{tételek} és a \emph{tautológiák} között.
...
Van egy formális módszer annak bemutatására, hogy egy mondat tétel: Bizonyítsa be. Minden sor esetében ellenőrizhetjük, hogy a sor megfelel-e a hivatkozott szabálynak. Nehéz lehet húsz soros bizonyítást előállítani, de nem olyan nehéz ellenőrizni a bizonyítás minden sorát, és igazolni annak helyességét - és ha a bizonyítás mindegyik sora külön-külön helyes, akkor az egész bizonyítás helyes. Annak igazolása, hogy egy mondat tautológia, angol nyelvű érvelést igényel minden lehetséges modellről. Nincs hivatalos mód annak ellenőrzésére, hogy az érvelés megalapozott-e. Ha választhatnánk annak bizonyítása között, hogy egy mondat tétel, illetve annak bizonyítása között, hogy tautológia, könnyebb megmutatni, hogy ez egy tétel.
...
Ezzel szemben nincs hivatalos módszer annak bemutatására, hogy a mondat \emph{nem} tétel. Magyar nyelven kellene érvelnünk minden lehetséges bizonyítékról. Van azonban egy hivatalos módszer annak bemutatására, hogy a mondat nem tautológia. Csak azt a modellt kell felépíteni, amelyben a mondat hamis. Ha választhatnánk annak bizonyítása között, hogy egy mondat nem tétel, és annak bizonyítása, hogy nem tautológia, könnyebb lenne megmutatni, hogy nem tautológia.
...
Szerencsére, egy mondat tétel akkor és csak akkor, ha tautológia. Ha bizonyítékot szolgáltatunk $\vdash\script{A}$, és így megmutatjuk, hogy ez egy tétel, akkor az következik, hogy \script{A} tautológia; vagyis $\models\script{A}$. Hasonlóképpen, ha olyan modellt állítunk elő, amelyben \script{A} hamis, és így megmutatjuk, hogy nem tautológia, akkor az következik, hogy \script{A} nem tétel.
...
Általában $\script{A}\vdash\script{B}$ akkor és csak akkor, ha $\script{A}\models\script{B}$. Mint ahogy:
\begin{itemize}
\item Az érv akkor és csak akkor \emph{érvényes}, ha \emph{a következtetés a felvetésből származtatható}.
\item Két mondat \emph{logikailag egyenértékű} akkor és csak akkor, ha \emph{bizonyíthatóan ekvivalensek}.
\item Egy mondatkészlet akkor és csak akkor \emph{konzisztens}, ha \emph{nem bizonyíthatóan inkonzisztens}.
\end{itemize}
...
Kiválaszthatja, hogy mikor gondolkodjon a bizonyítások terén, és mikor gondolkodjon modellek terén, azt használja amelyikkel egyszerűbb egy adott feladatot megoldani. A \ref{table.ProofOrModel} táblázat összefoglalja, mikor a legjobb bizonyításokat alkalmazni, mikor a legjobb modelleket.
...
Ily módon a bizonyítások és modellek sokoldalú eszközkészletet biztosítanak az érvek kezeléséhez. Ha le tudunk fordítani egy érvet QL-be, akkor annak logikai súlyát tisztán formális módon is meg tudjuk mérni. Ha deduktív módon érvényes, hivatalos igazolást adhatunk; ha érvénytelen, akkor nyújthatunk formális ellenpéldát.
